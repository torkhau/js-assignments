...
function getRectangleString(width, height) {
    ...
    const f = (char) => char.repeat(width - 2);
    const repeaded = f("─");
    const first = `┌${repeaded}┐\n`;
    const last = `└${repeaded}┘\n`
    const midle = `│${f(" ")}│\n`
    return first + midle.repeat(height - 2) + last;
}
...
function encodeToRot13(str) {
    const ROT13 = (char) => {
        ...
        return String.fromCharCode(((charCode & 223) - 52) % 26 + (charCode & 32) + 65)
    }
...
function getCardId(value) {
    ...
    const suit = ['♣', '♦', '♥', '♠'];
    return card.indexOf(value.slice(0, -1)) + suit.indexOf(value.slice(-1)) * 13;
}
